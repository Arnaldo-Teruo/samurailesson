	...
	var app = new Vue({
		...
		methods: {
			...
			clickMenu: function(v) {
				...
				localStorage.setItem('planActive', v.slug);
			},
			...
			setEndTime: function() {
				let startDateHour = this.startTime.split(":")[0];
				if (startDateHour == '') {
					return false;
				}
				let startDateMinute = this.startTime.split(":")[1];
				let startDateTime   = new Date(2000, 1, 1, startDateHour, startDateMinute, 0);
				let endDateTime     = startDateTime.setMinutes(startDateTime.getMinutes() + this.selectedPlanTime);
				let endDateHour     = new Date(endDateTime).getHours();
				endDateHour         = ('0' + endDateHour).slice(-2);
				let endDateMinute   = new Date(endDateTime).getMinutes();
